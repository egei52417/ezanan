...
exports.run = async (client, message, args) => {
  ...
  if (veri2) {
    const embed = new Discord.MessageEmbed()
      .setDescription(`Maksimum Rütbe Sınırına Ulaşılmış!`)
      .setColor("RED")
  ...
  if (!veri) {
    ...
    if (!enis) {
      const embed = new Discord.MessageEmbed()
        .setDescription(
          `Bir Davet Sayısı Belirtiniz!\nÖrnek: ${prefix}rütbe-ekle @OWNER 10`
        )
        .setColor("RED")
        .setFooter(client.user.username, client.user.avatarURL);
      message.channel.send(embed);
    ...
    const embed = new Discord.MessageEmbed()
      .setDescription(
        `Başarıyla ${sine} Rolü ${enis} Davet Karşılığında Sahip Olunabilecek!`
      )
      .setColor("GREEN")
      .setFooter(client.user.username, client.user.avatarURL);
    message.channel.send(embed);
